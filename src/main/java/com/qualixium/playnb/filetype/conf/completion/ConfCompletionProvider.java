...
import com.qualixium.playnb.filetype.conf.ConfLanguageHelper;
...
@MimeRegistration(mimeType = ConfLanguage.MIME_TYPE, service = CompletionProvider.class)
public class ConfCompletionProvider implements CompletionProvider {

    private static final char CHAR_TO_START_COMPLETION = ' ';

    @Override
    public CompletionTask createTask(int queryType, JTextComponent jtc) {
        Project owner = FileOwnerQuery.getOwner(MiscUtil.getFileObject(jtc.getDocument()));
            ...
            return new AsyncCompletionTask(new AsyncCompletionQuery() {
                ...
                protected void query(CompletionResultSet crs, Document dcmnt, int caretOffset) {
                    ...
                    try {
                        ...
                        List<ConfigurationItem> items = ConfLanguageHelper.getKeys();

                        items.stream()
                                .filter((confItem) -> (confItem.key.startsWith(filter)))
                                .forEach((confItem) -> {
                                    crs.addItem(new ConfCompletionItem(confItem, startOffset, caretOffset));
                                });
