...
public class RoutesMethodMethodCompletionItem implements CompletionItem {
    ...
    public RoutesMethodMethodCompletionItem(String text, int startOffset, int caretOffset) {
        this.text = text;
    ...
    public void defaultAction(JTextComponent jtc) {
        try {
            ...
            if (text.contains("(")) {
                int selectionEnd;
                if (text.contains(",")) {
                    selectionEnd = startOffset + text.indexOf(",");
                } else {
                    selectionEnd = startOffset + text.length() - 1;
                }
                jtc.setSelectionStart(startOffset + text.indexOf("(") + 1);
                jtc.setSelectionEnd(selectionEnd);
            }
